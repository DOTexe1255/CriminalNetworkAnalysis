...
export default function GraphView({
  ...
  const [entityMode, setEntityMode] = useState(true);
  ...
  useEffect(() => {
    ...
    async function loadGraph() {
      try {
        ...
        const useEntityGraph = Boolean(focusPersonId) && entityMode;
        const url = useEntityGraph
        ...
        const cy = cytoscape({
          ...
          style: [
            ...
            {
              selector: "edge.relationship-focused",
              style: { width: 3, "line-color": "#D9A441", opacity: 1 },
            },
            ...
            },
            {
              ...
              style: { "border-width": 2, "border-color": "#C7CDD6" },
            },
            {
              selector: "node.hovered",
              style: { label: "data(label)", "border-width": 2, "border-color": "#AEB7C2" },
            },
            {
  ...
  }, [focusPersonId, hopDistance, caseId, entityMode, onPersonSelect]);

  useEffect(() => {
    const cy = cyRef.current;
    if (!cy || !focusPersonId) return;
    ...
    const neighborhood = person.closedNeighborhood();
    person.addClass("focused");
    neighborhood.nodes().not(person).addClass("related");
